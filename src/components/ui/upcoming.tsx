  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 flex flex-col items-end gap-2">
      {isExpanded && notifications.length > 0 && (
        <div className="w-[min(24rem,calc(100vw-2rem))] rounded-2xl border border-gray-700/70 bg-black/90 p-3 shadow-2xl backdrop-blur-xl">
          <div className="mb-2 flex items-center justify-between">
            ...
            <button
              ...
              className="rounded-lg border border-gray-600 px-2 py-1 text-[10px] font-black uppercase tracking-widest text-gray-300 hover:border-gray-400"
            >
              Clear all
            </button>
          </div>
          ...
          <div className="max-h-[60vh] space-y-2 overflow-y-auto pr-1">
            {notifications.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className={`relative w-full rounded-xl border p-3 text-left text-white transition hover:brightness-110 ${stageStyles[item.stage]}`}
                >
                  <button
                    type="button"
                    onClick={() => dismissWithOptionalMark(item)}
                    className="absolute right-2 top-2 rounded-md border border-white/20 px-1.5 py-0.5 text-[10px] font-black text-white/80 hover:bg-white/10"
                  ...
                  >
                    ...
                    <p className="mt-1 text-xs text-gray-200/90">{item.body}</p>
                    <p className="mt-2 text-[10px] font-bold uppercase tracking-[0.15em] text-gray-300">
                      {item.stage === "todaySummary"
                        ? "Dashboard summary"
      ...
      {canShowBubble && (
        <button
          ...
          className="rounded-full border border-sky-500/50 bg-sky-600 px-4 py-2 text-xs font-black uppercase tracking-widest text-white shadow-lg shadow-sky-900/30"
        >
